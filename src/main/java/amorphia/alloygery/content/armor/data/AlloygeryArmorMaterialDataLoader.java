...
public class AlloygeryArmorMaterialDataLoader implements SimpleSynchronousResourceReloadListener
{
	...
	public void reload(ResourceManager manager)
	{
		...
		manager.findResources("armor_materials", path -> path.getPath().endsWith(".json")).forEach((identifier, resource) -> {
			...
			try (InputStream is = resource.getInputStream())
			{
				...
				AlloygeryArmorMaterialDataHelper.getArmorMaterialDataFromJson(jsonObject).ifPresentOrElse(materialData -> AlloygeryArmorMaterialRegistry.load(trimmedIdentifier, materialData),
