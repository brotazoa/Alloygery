...
public class AlloygeryToolMaterialDataLoader implements SimpleSynchronousResourceReloadListener
{
	...
	public void reload(ResourceManager manager)
	{
		...
		manager.findResources("tool_materials", path -> path.getPath().endsWith(".json")).forEach((identifier, resource) -> {
			...
			try (InputStream is = resource.getInputStream())
			{
				...
				AlloygeryToolMaterialDataHelper.getToolMaterialDataFromJson(jsonObject).ifPresentOrElse(data -> AlloygeryToolMaterialRegistry.load(trimmedIdentifier, data),
